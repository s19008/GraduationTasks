...
$blog = new Blog();
$blogData = $blog->getBycategory($_GET['category']);
...
if ($_GET) {
    try {
        ...
        $search_word = $_GET['category'];
        if ($search_word == "") {
            echo "文字を入力してください";
        } else {
            $sql = "select * from blog where category = " . $search_word . "";
            $sth = $dbh->prepare($sql);
            $sth->execute();
            $result = $sth->fetchAll();
            if ($result) {
...
<body>
    ...
    <section class="post">
        <h2 class="post-title">
            <?php echo h($blog->setcategoryName($blogData['category'])) ?>チャットルーム一覧
        </h2>
